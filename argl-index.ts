...
dotenv.config({ path: envCandidates.find(candidate => fs.existsSync(candidate)) ?? envCandidates[0] });

import { Client, EmbedBuilder, GatewayIntentBits, Message, MessageType, Events, Collection, Partials, User } from 'discord.js';
import config from './config';
import UserSchema, { UserDocument } from './mongodb-schemas/User';
import ABUSE_REASONS from './Constants';
import MessageLog from './mongodb-schemas/MessageLog';
import Milestones, { Milestone } from './Milestones';
import mongoose from 'mongoose';
import cron from 'node-cron';
import { getNumberWithOrdinal, createScoreboard } from './utils';
import type { Command } from './types';

const { token, mongo_uri, generalChatId, botId, guildId, arglTimeout, botChannelId, timeoutFeatureflag } = config;
let { currentSeason } = config;
...
function sendToChannel(channelId: string, content: string) {
...
client.on(Events.ClientReady, async () => {
    ...
    const tasks = cron.getTasks() as unknown as { length: number; [index: number]: { stop(): void }; splice(start: number, deleteCount: number): void };
...
client.on(Events.MessageCreate, async (message) => {
    if (message.content.toUpperCase() === 'ARGL' && !message.author.bot) {
        if (message.type === MessageType.Reply) {
            ...
            if (message.author.id === originalMessage.author.id) {
            ...
            } else if ((await UserSchema.checkUsersTimeoutStatus(message.author.id))!.isUserTimedOut === true) {
                ...
                // Add score to user
                const messageAuthor = (await message.fetchReference()).author;
                const dbUser = await UserSchema.increaseScore(isNameChangeArgl ? oldMemberId : messageAuthor.id, currentSeason);
                ...
                MessageLog.logArglMessage(message, originalMessage);
                ...
                if (timeoutFeatureflag) {
                    ...
                    setTimeout(async () => await UserSchema.unTimeOutUser(message.author.id), Number(arglTimeout));
                }
            }
        } else {
            sendToChannel(message.channelId, `I know you're in stitches right now, but don't forget: you need to **reply** to the person you're laughing at for this to count!`);
        }
    }
});
...
client.on(Events.MessageUpdate, async (oldMessage, newMessage) => {
    if (!oldMessage.content?.toUpperCase().includes('ARGL') && newMessage.content?.toUpperCase() === 'ARGL') {
        nameAndShameUser(newMessage as Message, ABUSE_REASONS.PREV_EDIT);
    }
});
...
client.on(Events.GuildMemberUpdate, async (oldMember, newMember) => {
    if (oldMember.displayName !== newMember.displayName) {
        sendToChannel(generalChatId, `"${oldMember.user.username}" changed their name to ${newMember.user.toString()}`);
    }
});

async function nameAndShameUser(abusingMessage: Message, abuseReason: string) {
...
async function modifyContentForNameChange(originalMessage: Message) {
...
function isMessageMilestone(user: UserDocument): Milestone | false {
...
function replyToArglMessage(message: Message, scoreboard: string, dbUser: UserDocument, messageAuthor: User) {
    ...
    if (milestone) {
        ...
        message.reply(`@everyone\n\nWe have a genuine "argl" in the chat. Remain calm! ${scoreboard}`);
    }
}
